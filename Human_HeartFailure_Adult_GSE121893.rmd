---
title: "GSE121893_failed_human_adult_heart_analysis"
author: "Cass Li"
date: "20/08/2020"
...
GSE121893_files.txt - list of fastq files uploaded to SRA
GSE121893_human_heart_sc_info.txt - metadata
human_heart_sc_umi.csv - umi
...
<div class = "teal">
```{r cache = TRUE}
ori_meta <- read.table("~/Documents/Project_scRNA_seq_meta_analysis/GSE121893/GSE121893_human_heart_sc_info.txt", header = FALSE)
names(ori_meta) <- as.matrix(ori_meta[1, ])
ori_meta <- ori_meta[-1, ]
head(ori_meta)
# subset the data for endothelial cells - EC
ori_meta_EC <- ori_meta %>% filter (CellType == "EC")
# Need to pay attention here that the "Type" of the prep where EC were extracted can be "CM". It means a CM enriched prep not a CM exclusive prep. The "ID" column can be used to extract umi for EC specifically.
ori_umi <- read.csv("~/Documents/Project_scRNA_seq_meta_analysis/GSE121893/human_heart_sc_umi.csv")
# Subset the umi dataset using the ID in ori_meta_EC
# dplyr might give you a little hiccup here just becasue the select space will be masked by the biomart. Just reinstall the package and you will be fine.
ori_umi_EC <- ori_umi %>% select(X, as.character(ori_meta_EC$ID))
# The first column of the matrix is the gene names. Need to change it to the rownames.
rownames(ori_umi_EC) <- as.character(ori_umi_EC$X)
ori_umi_EC <- ori_umi_EC[, -1]
# Now the data is tidy and can be used for downstream analysis.
Human <- CreateSeuratObject(ori_umi_EC, project = "HumanHeartProject", min.cells = 3, min.features = 200)
Human[["percent.mt"]] <- PercentageFeatureSet(Human, pattern = "^MT-")
# fix the metadata
Human@meta.data$age <- "Adult"
Human@meta.data$species <- "Human"
Human@meta.data$condition <- "Heart failure"
...
png(filename = "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/HF_HumanQC.png", width = 600, height = 400)
...
new.Human@meta.data <- new.Human@meta.data %>% mutate (accessionNo = "GSE121893", species = "Human", age = "Adult", condition = "Heart failure") %>% mutate (group = paste0(condition, " ", species))
...
saveRDS(new.Human, file = "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human.rds")

png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/HF_Human_UMAP.png", res = 300, width = 2500, height = 2500)
...
png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_clustree.png", res = 300, width = 4000, height = 3000)
clustree(new.Human) # cluster tree visualisation
dev.off()

png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_clustree_stability.png", res = 300, width = 4000, height = 3000)
clustree(new.Human, node_colour = "sc3_stability") # cluster tree visualisation
dev.off()

# It seems resolution 1.2 is a good choice and change the clusters to the res 1.2 one
new.Human@meta.data$seurat_clusters <- new.Human@meta.data$SCT_snn_res.1.2
new.Human <- FindClusters(new.Human, resolution = 1.2) # run it again

png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_clustering_Human.png", res = 300, width = 2500, height = 2500)
...
write.csv(Human_markers_top10, file = "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_cluster_marker_top10.csv")
...
for (i in 0:(length - 1)) { # starting from 0
        ...
        # Save GO table
        write.csv(GO_table, file = paste0("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_cluster_", i,"GO_term_table", ".csv"))
        ...
        png(paste0("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_cluster_", i,"GO_term_table", ".png"), res = 300, width = 4000, height = 1000)
...
png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_PECAM1_feature_plot.png", res = 300, width = 2500, height = 2500)
...
<div class = "teal">
```{r feature_plot, warning = FALSE, message = FALSE}
saveRDS(new.Human, "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_object.rds") # save object
...
cells_rankings
save(cells_rankings, file = "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_EC_rankings.RData")
...
save(cells_AUC, file = "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_EC_AUC.RData")
...
png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_SCTassay_AUCell_threshold.png", res = 300, width = 1500, height = 1500)
...
png("~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_SCTassay_AUCell_EC.png", res = 300, width = 2750, height = 2500)
...
saveRDS(Human_EC, "~/Documents/Project_scRNA_seq_meta_analysis/Healthy_HeartFailure_Human_comparison/GSE121893_HF_Human_EC.rds")
```
</div>
