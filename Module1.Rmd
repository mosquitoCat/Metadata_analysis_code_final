...
feature_up <- rownames(AllDEG_top50_up)[1:50]
feature_down <- rownames(AllDEG_top50_down) [1:50]
...
#########################################
#########################################
# 04052021 change "Normal" to "Uninjured"
integrated@meta.data <- integrated@meta.data %>% mutate(condition = case_when(condition == "Normal" ~ "Uninjured", condition == "Injured" ~ "Injured"))
saveRDS(integrated, file = "integrated.rds")
VlnPlot(integrated, features = feature_up, pt.size = 0, split.by = "condition", ncol = 6, cols = c("red", "blue"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)

# try remake the datafrme to using a different plotting package
gene_expression <- as.data.frame(t(integrated@assays$RNA@data[feature_up, ]))
#gene_expression <- as.data.frame(t(integrated@assays$RNA@data[feature_down, ]))
cell_metadata <- integrated@meta.data$condition
...
fig <- fig %>%
  layout(
    ...
    font = list(
      #family = "Open Sans",
      size = 16),
    margin = list(t = 50, l = 10)
  )

fig

# uninjured
p1 <- plot_ly(data = new_dataframe[cell_metadata == "Uninjured", ], x = ~ UMAP_1, y = ~ UMAP_2, type = "scatter", mode = "markers", color = ~ Selenok, colors = viridisLite::magma(20), marker = list(size = 10, line = list(color = "black", width = 0.5)), width = 500, height = 500) %>% layout(title = "", yaxis = list(zeroline = FALSE, showgrid = FALSE, showticklabels = FALSE, title = ""), xaxis = list(zeroline = FALSE, showgrid = FALSE, showticklabels = FALSE, title = ""), font = t, legend = list(x = 0.9, y = 0.1, font = list(size = 30))) %>% hide_colorbar() %>% hide_legend() %>% hide_guides()
# injured
p2 <- plot_ly(data = new_dataframe[cell_metadata == "Injured", ], x = ~ UMAP_1, y = ~ UMAP_2, type = "scatter", mode = "markers", color = ~ Selenok, colors = viridisLite::magma(20), marker = list(size = 10, line = list(color = "black", width = 0.5)), width = 500, height = 500) %>% layout(title = "", yaxis = list(zeroline = FALSE, showgrid = FALSE, showticklabels = FALSE, title = ""), xaxis = list(zeroline = FALSE, showgrid = FALSE, showticklabels = FALSE, title = ""), font = t, legend = list(x = 0.9, y = 0.1, font = list(size = 30))) %>% hide_colorbar() %>% hide_legend() %>% hide_guides()
...
common_down <- Reduce(intersect, list(rownames(MI1DHealthyAllDEG_down), rownames(MI3DHealthyAllDEG_down), rownames(MI7DHealthyAllDEG_down),  rownames(MI2WHealthyAllDEG_down), rownames(MI4WHealthyAllDEG_down), rownames(P6P2MI3WAllDEG_down)))
save(common_down, file = "common_down_regulated_genelist_inallinjured.Rdata")
...
x1 <- list(
  ...
  E = rownames(MI4WHealthyAllDEG_down),
  G = rownames(P6P2MI3WAllDEG_down)
  )
# "Sdpr"    "Slc28a2"
...
## Compare HealthyInjuredAllDEG_up with P6P2MI3WAllDEG_up
common_up <- Reduce(intersect, list(rownames(MI1DHealthyAllDEG_up), rownames(MI3DHealthyAllDEG_up), rownames(MI7DHealthyAllDEG_up),  rownames(MI2WHealthyAllDEG_up), rownames(MI4WHealthyAllDEG_up), rownames(P6P2MI3WAllDEG_up)))
save(common_up, file = "common_up_regulated_genelist_inallinjured.Rdata")
x2 <- list(
  ...
  E = rownames(MI4WHealthyAllDEG_up),
  G = rownames(P6P2MI3WAllDEG_down)
  )
# "Rps28" "Hspa8"

vennset2 <- overLapper(x2, type = "vennsets")
vennPlot(vennset2)
olBarplot(vennset2, mincount=1)
############
## common_down
############
expr <- integrated@assays$RNA@data
...
GO_table <- GenTable(GOdata, Fisher = resultFisher, topNodes = 20, numChar = 60)
# Save GO table
write.csv(GO_table, file = paste0("Common_down_allinjured_","GO_term_table", ".csv"))
# Generate GO term bar graph
Go_table_touse <- GO_table[1:15, ] # choose the top 5 GO terms
Go_table_touse$NewTerm <- paste0(Go_table_touse$GO.ID, "_", Go_table_touse$Term)
Go_table_touse$P_value <- -log10(as.numeric(Go_table_touse$Fisher))
...
png(paste0("Common_down_allinjured_", "GO_term_table", ".png"), res = 300, width = 2400, height = 1000)
ggplot(Go_table_touse, aes(x = NewTerm, y = P_value)) +
    geom_bar(stat = "identity", fill = "grey", width = 0.6) + 
    coord_flip() +
    ggtitle(paste0("Common_down_GO")) +
...
GO_table <- GenTable(GOdata, Fisher = resultFisher, topNodes = 20, numChar = 60)
# Save GO table
write.csv(GO_table, file = paste0("Common_up_allinjured_","GO_term_table", ".csv"))
# Generate GO term bar graph
Go_table_touse <- GO_table[1:15, ] # choose the top 5 GO terms
Go_table_touse$NewTerm <- paste0(Go_table_touse$GO.ID, "_", Go_table_touse$Term)
Go_table_touse$P_value <- -log10(as.numeric(Go_table_touse$Fisher))
...
png(paste0("Common_up_allinjured_", "GO_term_table", ".png"), res = 300, width = 2000, height = 1000)
ggplot(Go_table_touse, aes(x = Term, y = P_value)) +
    geom_bar(stat = "identity", fill = "violetred", width = 0.6) + 
    coord_flip() +
    ggtitle(paste0("Common_up_GO")) +
...
DotPlot(integrated, features = common_up, group.by = "group", cols = c("Navy", "plum1")) + theme(axis.text.x = element_text(angle = 45, hjust = 1, family = "Open Sans"))
ggsave("Dotplot_common_up_allinjured.png", dpi = 300, height = 3, width = 12, units = "in")

DotPlot(integrated, features = common_down, group.by = "group", cols = c("Navy", "plum1")) + theme(axis.text.x = element_text(angle = 45, hjust=1))
ggsave("Dotplot_common_down_allinjured.png", dpi = 300, height = 4, width = 18, units = "in")

DotPlot(integrated, features = c("Sdpr", "Slc28a2", "Rps28", "Hspa8"), group.by = "group", cols = c("Navy", "plum1")) + theme(axis.text.x = element_text(angle = 45, hjust=1))
# My Friday work stops here
ggsave("strictly_common_allinjured.png", dpi = 300, height = 4, width = 6, units = "in")

colours <- c ("#bfbfff", "#7879ff", "#dc1c13", "#0000ff", "#f6bdc0", "#f1959b", "#f07470", "#ea4c46", "darkred")
VlnPlot(integrated, features = c("Sdpr", "Slc28a2", "Rps28", "Hspa8"), group.by = "group", ncol = 2, cols = colours, pt.size = 0) + theme(axis.text.x = element_text(angle = 45, hjust=1))
ggsave("Vlnplot_strictly_common_allinjured.png", dpi = 300, height = 5, width = 10, units = "in")
```
</div>

### Step 16: Module 1c identify developmental regulons reactivated in injured adult heart

For this part, I'm going to look at TF and their regulatory targets rather than looking at single genes.
...
<div class = "teal">
```{r Module 1c identify developmental regulons reactivated in injured adult heart, message = FALSE, warning = FALSE}
DefaultAssay(integrated) <- "RNA"
Idents(integrated) <- "group"
...
## group
colours <- c ("#bfbfff", "#7879ff", "#dc1c13", "#0000ff", "#f6bdc0", "#f1959b", "#f07470", "#ea4c46", "darkred")
p1 <- plot_ly(data = df, x = ~ UMAP_1, y = ~ UMAP_2, type = "scatter", mode = "markers", color = ~ group, colors = colours, marker = list(size = 12, line = list(color = "black", width = 1)), width = 1000, height = 1000) %>% layout(title = "", yaxis = list(zeroline = FALSE), xaxis = list(zeroline = FALSE), font = t, legend = list(x = 0.9, y = 0.1, font = list(size = 30))) %>% hide_colorbar() %>% hide_legend() %>% hide_guides()

## group
p2 <- plot_ly(data = df, x = ~ UMAP_1, y = ~ UMAP_2, type = "scatter", mode = "markers", color = ~ seurat_clusters, colors = RColorBrewer::brewer.pal(11, "Set3"),marker = list(size = 12, line = list(color = "black", width = 1)), width = 1000, height = 1000) %>% layout(title = "", yaxis = list(zeroline = FALSE), xaxis = list(zeroline = FALSE), font = t, legend = list(x = 0.9, y = 0.1, font = list(size = 30))) %>% hide_colorbar() %>% hide_legend() %>% hide_guides()

# pie chart df
dfy <- df %>% group_by(seurat_clusters) %>% summarise(count = n()) %>% mutate (percentage = count/nrow(df)*100)
## pie chart
figy_pie <- plot_ly(dfy, labels = ~ seurat_clusters, values = ~ percentage, type = 'pie', marker = list(colors = RColorBrewer::brewer.pal(11, "Set3"), line = list(color = '#FFFFFF', width = 1)), textfont = list(size = 24), insidetextorientation = "radial")
...
summary <- summary %>% mutate (percentage_norm = case_when(seurat_clusters == "0" ~ percentage/summary1[1, 2]*100, seurat_clusters == "1" ~ percentage/summary1[2, 2]*100, seurat_clusters == "2" ~ percentage/summary1[3, 2]*100, seurat_clusters == "3" ~ percentage/summary1[4, 2]*100, seurat_clusters == "4" ~ percentage/summary1[5, 2]*100, seurat_clusters == "5" ~ percentage/summary1[6, 2]*100, seurat_clusters == "6" ~ percentage/summary1[7, 2]*100, seurat_clusters == "7" ~ percentage/summary1[8, 2]*100, seurat_clusters == "8" ~ percentage/summary1[9, 2]*100, seurat_clusters == "9" ~ percentage/summary1[10, 2]*100, seurat_clusters == "10" ~ percentage/summary1[11, 2]*100))
...
#########################
## evaluate tf activities
#########################
## find marker tfs
Idents(integrated) <- "seurat_clusters"
integrated.markers <- FindAllMarkers(integrated, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25, verbose = FALSE)
write.csv(integrated.markers, file = "marker_TFs.csv")

markers_top10 <- integrated.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_log2FC) %>% dplyr::filter (p_val_adj < 0.05)
write.csv(markers_top10, file = "top10marker_TFs.csv")

markers_top1 <- integrated.markers %>% group_by(cluster) %>% top_n(n = 1, wt = avg_log2FC) %>% dplyr::filter (p_val_adj < 0.05)
VlnPlot(integrated, features = c("Atf2", "Smad3", "Sox2", "Taf1", "Spi1"), stack = TRUE, flip = TRUE, cols = c("#FFFFB3", "#FCCDE5", "#D9D9D9", "#BC80BD", "#CCEBC5")) %>% hide_legend() 
ggsave("cluster_specific_tf.png", width = 5, height = 5)

## We transform Viper scores, scaled by seurat, into a data frame to better 
## handling the results
viper_scores_df <- GetAssayData(integrated, slot = "data", assay = "dorothea") %>% data.frame(check.names = F) %>% t()

## We create a data frame containing the cells and their groups
Idents(integrated) <- "paperID"
CellsClusters <- data.frame(cell = names(Idents(integrated)), 
                            cell_type = as.character(Idents(integrated)),
                            check.names = F)
...
viper_scores_clusters$cell_type <- factor(viper_scores_clusters$cell_type, levels = c("P6 Healthy", "P10 Healthy", "P2 MI3W", "Adult Healthy", "Adult MI1D", "Adult MI3D", "Adult MI7D", "Adult MI2W", "Adult MI4W"))
...
## We select the 30 most variable TFs. (30*9 populations = 270)
...
## plot tf activity
png("viper_heatmap.png", res = 300, width = 2500, height = 800)
ggheatmap(summarized_viper_scores_df, show_dendrogram = c(TRUE, FALSE))
dev.off()

progeny_hmap = pheatmap(summarized_viper_scores_df,fontsize=14, 
                        fontsize_row = 14, 
                         color=viridisLite::viridis(20), angle_col = 45,
                         treeheight_col = 40, treeheight_row = 40, border_color = "white",
...
### Step 17: Module 1c identify differentially activated TFs
...
<div class = "teal">
```{r Module 1c identify differentially activated TFs, message = FALSE, warning = FALSE}
DefaultAssay(integrated) <- "dorothea"
...
####################
# study all cells first
P6P10AllDER <- FindMarkers(integrated, ident.1 = "P6 Healthy", ident.2 = "P10 Healthy", min.pct = 0.25, verbose = FALSE)
write.csv(P6P10AllDER, file = "P6P10AllDER.csv")

# top 50
## downregulated in P6
P6P10AllDER_down <- P6P10AllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(P6P10AllDER_down, file = "downregulated_P6P10AllDER.csv")
P6P10AllDER_top50_down <- P6P10AllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(P6P10AllDER_top50_down)
## upregulated in P6
P6P10AllDER_up <- P6P10AllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(P6P10AllDER_up, file = "upregulated_P6P10AllDER.csv")
P6P10AllDER_top50_up <- P6P10AllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(P6P10AllDER_top50_up)

# plot up and down regulated genes
feature_up <- rownames(P6P10AllDER_top50_up)[1:10]
feature_down <- rownames(P6P10AllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "detailedage", idents = c("P6 Healthy", "P10"), ncol = 6, cols = c("#dc1c13", "#bfbfff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "detailedage", idents = c("P6 Healthy", "P10"), ncol = 6, cols = c("#dc1c13", "#bfbfff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))

####################
## P2 MI3W vs P6
####################
# study all cells first
#P6P2MI3WAllDER <- FindMarkers(integrated, ident.1 = "P6 Healthy", ident.2 = "P2 MI3W", min.pct = 0.25, verbose = FALSE)
Idents(integrated) <- "paperID"
P6P2MI3WAllDER <- FindMarkers(integrated, ident.1 = "P2 MI21D", ident.2 = "uninjured P6", min.pct = 0.25, verbose = FALSE)
write.csv(P6P2MI3WAllDER, file = "P6P2MI3WAllDER.csv")

# top 50
## downregulated in P2 MI21D
P6P2MI3WAllDER_down <- P6P2MI3WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0) %>% top_n(n = 500, wt = abs(avg_log2FC))
write.csv(P6P2MI3WAllDER_down, file = "downregulated_P6P2MI3WAllDER.csv")
P6P2MI3WAllDER_top50_down <- P6P2MI3WAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
#View(P6P2MI3WAllDER_top50_down)
write.csv(P6P2MI3WAllDER_top50_down, file = "top50_downregulated_P6P2MI3WAllDER.csv")

## upregulated in P2 MI21D
P6P2MI3WAllDER_up <- P6P2MI3WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0) %>% top_n(n = 500, wt = abs(avg_log2FC))
write.csv(P6P2MI3WAllDER_up, file = "upregulated_P6P2MI3WAllDER.csv")
P6P2MI3WAllDER_top50_up <- P6P2MI3WAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
#View(P6P2MI3WAllDER_top50_up)
write.csv(P6P2MI3WAllDER_top50_up, file = "top50_upregulated_P6P2MI3WAllDER.csv")

# plot up and down regulated genes
feature_up <- rownames(P6P2MI3WAllDER_top50_up)[1:10]
feature_down <- rownames(P6P2MI3WAllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "detailedage", idents = c("P6 Healthy", "P2 MI3W"), ncol = 6, cols = c("#dc1c13", "#bfbfff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "detailedage", idents = c("P6 Healthy", "P2 MI3W"), ncol = 6, cols = c("#dc1c13", "#bfbfff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))

##############################
## Adult MI vs Adult Healthy
##############################
Idents(integrated) <- "newgroup"

AdultInjuredVSUninjuredAllDER <- FindMarkers(integrated, ident.1 = "Adult Injured", ident.2 = "Adult Normal", min.pct = 0.25, verbose = FALSE)
write.csv(AdultInjuredVSUninjuredAllDER, file = "Adult_InjuredVSUninjured_AllDER.csv")

## Upegulated in injured
AdultInjuredVSUninjuredAllDER_up <- AdultInjuredVSUninjuredAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(AdultInjuredVSUninjuredAllDER_up, file = "up_Adult_Injured_VS_Uninjured_DER.csv")
AdultInjuredVSUninjuredAllDER_top50_up <- AdultInjuredVSUninjuredAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultInjuredVSUninjuredAllDER_top50_up)
write.csv(AdultInjuredVSUninjuredAllDER_top50_up, file = "top50_up_Adult_Injured_VS_Uninjured_DER.csv")

## Downegulated in injured
AdultInjuredVSUninjuredAllDER_down <- AdultInjuredVSUninjuredAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(AdultInjuredVSUninjuredAllDER_down, file = "down_Adult_Injured_VS_Uninjured_DER.csv")
AdultInjuredVSUninjuredAllDER_top50_down <- AdultInjuredVSUninjuredAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultInjuredVSUninjuredAllDER_top50_down)
write.csv(AdultInjuredVSUninjuredAllDER_top50_down, file = "top50_down_Adult_Injured_VS_Uninjured_DER.csv")
...
# study all cells first
Idents(integrated) <- "group"
AdultHealthyAdultMI1DAllDER <- FindMarkers(integrated, ident.1 = "Adult Healthy", ident.2 = "Adult MI1D", min.pct = 0.25, verbose = FALSE)
write.csv(AdultHealthyAdultMI1DAllDER, file = "AdultHealthyAdultMI1DAllDER.csv")

# top 50
## downregulated in AdultHealthy
AdultHealthyAdultMI1DAllDER_down <- AdultHealthyAdultMI1DAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(AdultHealthyAdultMI1DAllDER_down, file = "downregulated_AdultHealthyAdultMI1DAllDER.csv")
AdultHealthyAdultMI1DAllDER_top50_down <- AdultHealthyAdultMI1DAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI1DAllDER_top50_down)
## upregulated in AdultHealthy
AdultHealthyAdultMI1DAllDER_up <- AdultHealthyAdultMI1DAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(AdultHealthyAdultMI1DAllDER_up, file = "upregulated_AdultHealthyAdultMI1DAllDER.csv")
AdultHealthyAdultMI1DAllDER_top50_up <- AdultHealthyAdultMI1DAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI1DAllDER_top50_up)

# plot up and down regulated genes
feature_up <- rownames(AdultHealthyAdultMI1DAllDER_top50_up)[1:10]
feature_down <- rownames(AdultHealthyAdultMI1DAllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI1D"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI1D"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
...
# study all cells first
Idents(integrated) <- "group"
AdultHealthyAdultMI3DAllDER <- FindMarkers(integrated, ident.1 = "Adult Healthy", ident.2 = "Adult MI3D", min.pct = 0.25, verbose = FALSE)
write.csv(AdultHealthyAdultMI3DAllDER, file = "AdultHealthyAdultMI3DAllDER.csv")

# top 50
## downregulated in AdultHealthy
AdultHealthyAdultMI3DAllDER_down <- AdultHealthyAdultMI3DAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(AdultHealthyAdultMI3DAllDER_down, file = "downregulated_AdultHealthyAdultMI3DAllDER.csv")
AdultHealthyAdultMI3DAllDER_top50_down <- AdultHealthyAdultMI3DAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI3DAllDER_top50_down)
## upregulated in AdultHealthy
AdultHealthyAdultMI3DAllDER_up <- AdultHealthyAdultMI3DAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(AdultHealthyAdultMI3DAllDER_up, file = "upregulated_AdultHealthyAdultMI3DAllDER.csv")
AdultHealthyAdultMI3DAllDER_top50_up <- AdultHealthyAdultMI3DAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI3DAllDER_top50_up)

# plot up and down regulated genes
feature_up <- rownames(AdultHealthyAdultMI3DAllDER_top50_up)[1:10]
feature_down <- rownames(AdultHealthyAdultMI3DAllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI3D"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI3D"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
...
# study all cells first
Idents(integrated) <- "group"
AdultHealthyAdultMI7DAllDER <- FindMarkers(integrated, ident.1 = "Adult Healthy", ident.2 = "Adult MI7D", min.pct = 0.25, verbose = FALSE)
write.csv(AdultHealthyAdultMI7DAllDER, file = "AdultHealthyAdultMI7DAllDER.csv")

# top 50
## downregulated in AdultHealthy
AdultHealthyAdultMI7DAllDER_down <- AdultHealthyAdultMI7DAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(AdultHealthyAdultMI7DAllDER_down, file = "downregulated_AdultHealthyAdultMI7DAllDER.csv")
AdultHealthyAdultMI7DAllDER_top50_down <- AdultHealthyAdultMI7DAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI7DAllDER_top50_down)
## upregulated in AdultHealthy
AdultHealthyAdultMI7DAllDER_up <- AdultHealthyAdultMI7DAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(AdultHealthyAdultMI7DAllDER_up, file = "upregulated_AdultHealthyAdultMI7DAllDER.csv")
AdultHealthyAdultMI7DAllDER_top50_up <- AdultHealthyAdultMI7DAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI7DAllDER_top50_up)

# plot up and down regulated genes
feature_up <- rownames(AdultHealthyAdultMI7DAllDER_top50_up)[1:10]
feature_down <- rownames(AdultHealthyAdultMI7DAllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI7D"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI7D"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
...
# study all cells first
Idents(integrated) <- "group"
AdultHealthyAdultMI2WAllDER <- FindMarkers(integrated, ident.1 = "Adult Healthy", ident.2 = "Adult MI2W", min.pct = 0.25, verbose = FALSE)
write.csv(AdultHealthyAdultMI2WAllDER, file = "AdultHealthyAdultMI2WAllDER.csv")

# top 50
## downregulated in AdultHealthy
AdultHealthyAdultMI2WAllDER_down <- AdultHealthyAdultMI2WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(AdultHealthyAdultMI2WAllDER_down, file = "downregulated_AdultHealthyAdultMI2WAllDER.csv")
AdultHealthyAdultMI2WAllDER_top50_down <- AdultHealthyAdultMI2WAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI2WAllDER_top50_down)
## upregulated in AdultHealthy
AdultHealthyAdultMI2WAllDER_up <- AdultHealthyAdultMI2WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(AdultHealthyAdultMI2WAllDER_up, file = "upregulated_AdultHealthyAdultMI2WAllDER.csv")
AdultHealthyAdultMI2WAllDER_top50_up <- AdultHealthyAdultMI2WAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI2WAllDER_top50_up)

# plot up and down regulated genes
feature_up <- rownames(AdultHealthyAdultMI2WAllDER_top50_up)[1:10]
feature_down <- rownames(AdultHealthyAdultMI2WAllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI2W"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI2W"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
...
# study all cells first
Idents(integrated) <- "group"
AdultHealthyAdultMI4WAllDER <- FindMarkers(integrated, ident.1 = "Adult Healthy", ident.2 = "Adult MI4W", min.pct = 0.25, verbose = FALSE)
write.csv(AdultHealthyAdultMI4WAllDER, file = "AdultHealthyAdultMI4WAllDER.csv")

# top 50
## downregulated in AdultHealthy
AdultHealthyAdultMI4WAllDER_down <- AdultHealthyAdultMI4WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
write.csv(AdultHealthyAdultMI4WAllDER_down, file = "downregulated_AdultHealthyAdultMI4WAllDER.csv")
AdultHealthyAdultMI4WAllDER_top50_down <- AdultHealthyAdultMI4WAllDER_down %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI4WAllDER_top50_down)
## upregulated in AdultHealthy
AdultHealthyAdultMI4WAllDER_up <- AdultHealthyAdultMI4WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
write.csv(AdultHealthyAdultMI4WAllDER_up, file = "upregulated_AdultHealthyAdultMI4WAllDER.csv")
AdultHealthyAdultMI4WAllDER_top50_up <- AdultHealthyAdultMI4WAllDER_up %>% top_n(n = 50, wt = abs(avg_log2FC))
View(AdultHealthyAdultMI4WAllDER_top50_up)

# plot up and down regulated genes
feature_up <- rownames(AdultHealthyAdultMI4WAllDER_top50_up)[1:10]
feature_down <- rownames(AdultHealthyAdultMI4WAllDER_top50_down) [1:10]

p_up <- VlnPlot(integrated, features = c(feature_up), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI4W"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10) + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
p_down <- VlnPlot(integrated, features = c(feature_down), pt.size = 0, split.by = "group", idents = c("Adult Healthy", "Adult MI4W"), cols = c("#f6bdc0", "#0000ff"), same.y.lims = TRUE, stack = TRUE, flip = TRUE, y.max = 10)  + theme(axis.text.x = element_text(angle = 0, hjust = 0.5))
...
common_down <- Reduce(intersect, list(rownames(P6P2MI3WAllDER_down), rownames(AdultHealthyAdultMI1DAllDER_down), rownames(AdultHealthyAdultMI3DAllDER_down),  rownames(AdultHealthyAdultMI7DAllDER_down), rownames(AdultHealthyAdultMI2WAllDER_down), rownames(AdultHealthyAdultMI4WAllDER_down)))
save(common_down, file = "common_down_regulated_genelist_all.Rdata")
# Ehf, Foxo1 common of all
# Nanog, Ehf, Rfx1, Foxo1, Tcf7l2 without P2 3WMI
x1 <- list(
  #A = rownames(P6P2MI3WAllDER_down),
  B = rownames(AdultHealthyAdultMI1DAllDER_down),
  C = rownames(AdultHealthyAdultMI3DAllDER_down),
  D = rownames(AdultHealthyAdultMI7DAllDER_down),
  E = rownames(AdultHealthyAdultMI2WAllDER_down),
  G = rownames(AdultHealthyAdultMI4WAllDER_down)
  )
...
common_up <- Reduce(intersect, list(rownames(P6P2MI3WAllDER_up), rownames(AdultHealthyAdultMI1DAllDER_up), rownames(AdultHealthyAdultMI3DAllDER_up),  rownames(AdultHealthyAdultMI7DAllDER_up), rownames(AdultHealthyAdultMI2WAllDER_up), rownames(AdultHealthyAdultMI4WAllDER_up)))
save(common_up, file = "common_up_regulated_genelist_all.Rdata")
# Foxj2 common of all
# "Usf2"  "Ssrp1" "Myc"   "Usf1"  "Rel"   "Rela"  "Trp53" "Foxj2" "Atf2" "Sp1"   "Etv4"  "Ets2"  "Jun"   "Egr1"  "Gata2" without P2 3WMI

x2 <- list(
  #A = rownames(P6P2MI3WAllDER_up),
  B = rownames(AdultHealthyAdultMI1DAllDER_up),
  C = rownames(AdultHealthyAdultMI3DAllDER_up),
  D = rownames(AdultHealthyAdultMI7DAllDER_up),
  E = rownames(AdultHealthyAdultMI2WAllDER_up),
  G = rownames(AdultHealthyAdultMI4WAllDER_up)
  )
...
## plot the common DERs
DotPlot(integrated, features = c(common_up, common_down), group.by = "group", cols = c("Navy", "plum1")) + theme(axis.text.x = element_text(angle = 45, hjust = 1, family = "Open Sans"))
ggsave("Dotplot_common_up_and_down_allinjured_DER.png", dpi = 300, height = 4, width = 7, units = "in")
```
</div>

### Step 18: Module 1d DER between P6 vs P10 and P6 vs P2 3WMI
...
<div class = "teal">
```{r Module 1d DER between P6 vs P10 and P6 vs P2 3WMI, message = FALSE, warning = FALSE}
common_down <- Reduce(intersect, list(rownames(P6P2MI3WAllDER_down), rownames(P6P10AllDER_down)))
common_up <- Reduce(intersect, list(rownames(P6P2MI3WAllDER_up), rownames(P6P10AllDER_up)))
# this is not informative because P2 3WMI is similar to P10 in terms of cell maturation and differentiation and the similarity in genes/regulon expression doesn't mean much to proliferation or regeneration
```
</div>

### Step 19: Scrat metagene analysis

colours = c ("#bfbfff", "#7879ff", "#dc1c13", "#0000ff", "#f6bdc0", "#f1959b", "#f07470", "#ea4c46", "darkred")

levels = c("Uninjured P6", "Uninjured P10", "P2 MI3W", "Uninjured dult", "Adult MI1D", "Adult MI3D", "Adult MI7D", "Adult MI2W", "Adult MI4W"))

<div class = "teal">
```{r Scrat metagene analysis, message = FALSE, warning = FALSE}
### use integrated data
DefaultAssay(integrated) <- "RNA"
df <- as.matrix(integrated@assays$RNA@data)
df1 <- as.data.frame(t(df))
save(df1, file = "data_for_scrat.RData")

integrated@meta.data <- integrated@meta.data %>% mutate(color = case_when(group == "P6 Healthy" ~ "#bfbfff", group == "P10 Healthy" ~ "#7879ff", group == "P2 MI3W" ~ "#dc1c13", group == "Adult Healthy" ~ "#0000ff", group == "Adult MI1D" ~ "#f6bdc0", group == "Adult MI3D" ~ "#f1959b", group == "Adult MI7D" ~ "#f07470", group == "Adult MI2W" ~ "#ea4c46", group == "Adult MI4W" ~ "darkred"))
df2 <- data.frame(cell_id = rownames(df1), experiment = "Mouse_Meta", assignment_LB = integrated@meta.data$group, color = integrated@meta.data$color, row.names = rownames(df1))
...
dfFinal <- t(dfFinal) 
#save(dfFinal, file = "dfFinal_for_scrat.RData")
# Set up the environment
env <- scrat.new(list(dataset.name = "mouse_meta",
                      dim.1stLvlSom = 50,
                      dim.2ndLvlSom = 20,
                      ...
                      pseudotime.estimation = list(
                              ...
                              k = 30,
                              I = 5,
                              initiator.sample = "Adult Healthy"),
...
### Step 20: Check mouse genes in human data

<div class = "teal">
```{r checking genes, message = FALSE, warning = FALSE}
list_to_check <- c("Sox18", "Rbpms", "Rpl3", "Hspa8", "Rps28", "Nedd9", "Cdh13", "Slc25a4", "Rpl6", "H2afj", "H2-D1", "Nptn", "Gm26917", "Wwc2", "Prkch", "Eef1g", "Ube2k", "Tsc22d3", "Rps6", "Rpl10a", "Gpi1", "Actg1", "Mat2a", "Oaz1", "Adam15", "Eef2", "Foxn3", "Pecam1", "Ctsd", "Mmp15", "Cav2", "Rhob", "Igfbp7", "Mylip")

df <- mouse2human(list_to_check)
new_list <- df$humanGene

DotPlot(human_integrated, features = c(new_list, "FOXN3", "NPTN"), group.by = "newgroup", cols = c("navy", "plum1")) + theme_bw() + theme(axis.text.x = element_text(angle = 90))
```
</div>

### Step 21: Change the colour of Dimplot 22072021 and 16/09/2021

<div class = "teal">
```{r redimplot, message = FALSE, warning = FALSE}
mouse_integrated@meta.data <- mouse_integrated@meta.data %>% mutate(paperID = case_when(paperID == "uninjured P6" ~ "Uninjured P6", paperID == "uninjured P10" ~ "Uninjured P10", paperID == "uninjured adult" ~ "Uninjured adult", paperID == "P2 MI21D" ~ "P2 21 days post MI", paperID == "Adult MI1D" ~ "Adult 1 day post MI", paperID == "Adult MI3D" ~ "Adult 3 days post MI", paperID == "Adult MI7D" ~ "Adult 7 days post MI", paperID == "Adult MI14D" ~ "Adult 14 days post MI", paperID == "Adult MI28D" ~ "Adult 28 days post MI"))
saveRDS(mouse_integrated, file = "mouse_integrated.rds")
Idents(mouse_integrated) <- "paperID"

# It's very important to have an overall view of the dataset and for this purpose, I build a new data frame using the UMAP coordinates and the metadata.
df <- cbind(mouse_integrated@reductions$umap@cell.embeddings, mouse_integrated@meta.data)
# Plotly is used to generate beautiful feature plot here.
t <- list(
  family = "Helvetica",
  size = 16,
...
# coloured by paperID
p1 <- plot_ly(data = df, x = ~ UMAP_1, y = ~ UMAP_2, type = "scatter", mode = "markers", color = ~ paperID, colors = colorRampPalette(brewer.pal(9,"Set1"))(9), marker = list(size = 12, line = list(color = "black", width = 1)), width = 1000, height = 1000) %>% layout(title = "", yaxis = list(zeroline = FALSE), xaxis = list(zeroline = FALSE), font = t, legend = list(x = 0.9, y = 0.1, font = list(size = 30))) %>% hide_colorbar() %>% hide_legend() %>% hide_guides()

# pie chart for groups
df$paperID <- factor(df$paperID, levels = c("Uninjured P6", "Uninjured P10", "P2 21 days post MI", "Uninjured adult", "Adult 1 day post MI", "Adult 3 days post MI", "Adult 7 days post MI", "Adult 14 days post MI", "Adult 28 days post MI"))

## pie chart
dfy <- df %>% group_by(paperID) %>% summarise(count = n()) %>% mutate (percentage = count/nrow(df)*100)

dfy$paperID <- factor(dfy$paperID, levels = c("Uninjured P6", "Uninjured P10", "P2 21 days post MI", "Uninjured adult", "Adult 1 day post MI", "Adult 3 days post MI", "Adult 7 days post MI", "Adult 14 days post MI", "Adult 28 days post MI"))

figy_pie <- plot_ly(dfy, labels = ~ paperID, values = ~ percentage, type = 'pie', marker = list(colors = colorRampPalette(brewer.pal(9,"Set1"))(9), line = list(color = '#FFFFFF', width = 1)), textfont = list(size = 24, family = "Helvetica"), insidetextorientation = "radial")

dfy <- df %>% group_by(seurat_clusters) %>% summarise(count = n()) %>% mutate (percentage = count/nrow(df)*100)
figy_pie <- plot_ly(dfy, labels = ~ seurat_clusters, values = ~ percentage, type = 'pie', marker = list(colors = further.colour, line = list(color = '#FFFFFF', width = 1)), textfont = list(size = 24, family = "Helvetica"), insidetextorientation = "radial") %>% hide_colorbar() %>% hide_legend() %>% hide_guides()

######################################
## redo cluster composition
# select the feature to analyse
meta_to_use <- mouse_integrated@meta.data[, c("paperID", "seurat_clusters")]

# summarise the parametres
summary <- meta_to_use %>% group_by(paperID, seurat_clusters) %>% tally()
summary <- as.data.frame(summary)

# number of cells in each group
group_number <- as.data.frame(meta_to_use %>% group_by(paperID) %>% tally())

# calculate the percentage of cells in each cluster
summary <- summary %>% mutate (percentage = case_when(paperID == "Uninjured P6" ~ n/group_number[1, 2]*100, paperID == "Uninjured P10" ~ n/group_number[2, 2]*100, paperID == "P2 21 days post MI" ~ n/group_number[3, 2]*100, paperID == "Uninjured adult" ~ n/group_number[4, 2]*100, paperID == "Adult 1 day post MI" ~ n/group_number[5, 2]*100, paperID == "Adult 3 days post MI" ~ n/group_number[6, 2]*100, paperID == "Adult 7 days post MI" ~ n/group_number[7, 2]*100, paperID == "Adult 14 days post MI" ~ n/group_number[8, 2]*100, paperID == "Adult 28 days post MI" ~ n/group_number[9, 2]*100))

# normalise percentage data
summary1 <- as.data.frame(summary %>% group_by(seurat_clusters) %>% summarise(norm_total = sum(percentage)))
summary <- summary %>% mutate (percentage_norm = case_when(seurat_clusters == "0" ~ percentage/summary1[1, 2]*100, seurat_clusters == "1" ~ percentage/summary1[2, 2]*100, seurat_clusters == "2" ~ percentage/summary1[3, 2]*100, seurat_clusters == "3" ~ percentage/summary1[4, 2]*100, seurat_clusters == "4" ~ percentage/summary1[5, 2]*100, seurat_clusters == "5" ~ percentage/summary1[6, 2]*100, seurat_clusters == "6" ~ percentage/summary1[7, 2]*100, seurat_clusters == "7" ~ percentage/summary1[8, 2]*100, seurat_clusters == "8" ~ percentage/summary1[9, 2]*100, seurat_clusters == "9" ~ percentage/summary1[10, 2]*100, seurat_clusters == "10" ~ percentage/summary1[11, 2]*100, seurat_clusters == "11" ~ percentage/summary1[12, 2]*100, seurat_clusters == "12" ~ percentage/summary1[13, 2]*100, seurat_clusters == "13" ~ percentage/summary1[14, 2]*100, seurat_clusters == "14" ~ percentage/summary1[15, 2]*100))

summary$paperID <- factor(summary$paperID, levels = c("Uninjured P6", "Uninjured P10", "P2 21 days post MI", "Uninjured adult", "Adult 1 day post MI", "Adult 3 days post MI", "Adult 7 days post MI", "Adult 14 days post MI", "Adult 28 days post MI"))

write.csv(summary, file = "cluster_composition.csv")

# choose a reference group
Adult_data <- summary %>% filter (paperID == "Uninjured adult")
Adult_table <- Adult_data[, c(2, 5)]
cluster_levels <- Adult_table[order(Adult_table$percentage_norm), ]$seurat_clusters
summary$seurat_clusters <- factor(summary$seurat_clusters,levels = cluster_levels)
...
# normalised percentage graph
#png("cluster_normalised_percentage_group_split.png", res = 300, width = 2000, height = 2000)
p <- ggplot(data = summary, aes(x = seurat_clusters, y = percentage_norm)) + geom_bar(stat = "identity", aes(fill = paperID)) + scale_fill_manual(values = colorRampPalette(brewer.pal(9,"Set1"))(9)) + theme_classic() + labs(x = "Cluster", y = "Ordered Normalised Percentage (%)") + theme(text = element_text(family = "Helvetica"), legend.position = c(0.85, 0.8), legend.key.size = unit(1.5,"line"), legend.text = element_text(size = 24), legend.title = element_text(size = 24), axis.line = element_line(colour = "black", size = 1.25, linetype = "solid"), axis.text.x = element_text(size = 24, colour = "black"), axis.text.y = element_text(size = 24, colour = "black"), axis.title.x = element_text(size = 24), axis.title.y = element_text(size = 24), axis.ticks.length = unit(0.4,"cm")) + NoLegend()
ggplotly(p, width = 800, height = 800)
#dev.off()
```
</div>
