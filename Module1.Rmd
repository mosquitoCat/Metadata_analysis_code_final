...
viper_scores_df <- GetAssayData(integrated, slot = "scale.data", assay = "dorothea") %>% data.frame(check.names = F) %>% t()
...
# top 50
## downregulated in P6
P6P2MI3WAllDER_down <- P6P2MI3WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC > 0)
...
## upregulated in P6
P6P2MI3WAllDER_up <- P6P2MI3WAllDER %>% dplyr::filter (p_val_adj < 0.05, avg_log2FC < 0)
...
### use integrated data
DefaultAssay(integrated) <- "RNA"
df1 <- as.data.frame(t(as.matrix(integrated@assays$RNA@data)))
